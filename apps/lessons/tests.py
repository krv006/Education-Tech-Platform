...
class AITutorTests(APITestCase):
    """AI test qoralamasi + o'quvchi savol-javobi — Gemini chaqiruvi mock qilinadi."""
    ...
    def setUp(self):
        ...
        self.teacher = mk('ai_t', User.Role.TEACHER)
        self.student = mk('ai_s', User.Role.STUDENT)
        self.course = Course.objects.create(teacher=self.teacher, title='Algebra', subject='Matematika')
        Enrollment.objects.create(course=self.course, student=self.student, status=Enrollment.Status.APPROVED)
        self.finished_lesson = Lesson.objects.create(
            course=self.course, title='Kvadrat tenglamalar', starts_at=timezone.now() - timedelta(days=1),
    ...
    @patch('apps.lessons.services.ai.generate_quiz_draft', return_value=FAKE_QUIZ_DRAFT)
    def test_teacher_gets_ai_quiz_draft_from_finished_lessons(self, mock_draft):
        resp = self.api(self.teacher).post('/api/v1/quizzes/ai-draft/', {
            'course': str(self.course.id), 'question_count': 3,
        }, format='json')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json(), FAKE_QUIZ_DRAFT)
        _, kwargs = mock_draft.call_args
        self.assertEqual(kwargs['subject_text'], 'Matematika')
        self.assertEqual(kwargs['lesson_titles'], ['Kvadrat tenglamalar'])
        self.assertEqual(kwargs['question_count'], 3)

    def test_ai_quiz_draft_fails_without_finished_lessons(self):
        from .models import Course

        empty_course = Course.objects.create(teacher=self.teacher, title='Bo\'sh kurs', subject='Fizika')
        resp = self.api(self.teacher).post('/api/v1/quizzes/ai-draft/', {'course': str(empty_course.id)}, format='json')
        self.assertEqual(resp.status_code, 400)

    def test_student_cannot_request_ai_quiz_draft(self):
        resp = self.api(self.student).post('/api/v1/quizzes/ai-draft/', {'course': str(self.course.id)}, format='json')
        self.assertEqual(resp.status_code, 403)

    def test_other_teacher_cannot_generate_draft_for_foreign_course(self):
        from apps.accounts.models import User

        other = User(username='ai_t2', role=User.Role.TEACHER)
        other.set_password('x')
        other.save()
        resp = self.api(other).post('/api/v1/quizzes/ai-draft/', {'course': str(self.course.id)}, format='json')
        self.assertEqual(resp.status_code, 404)

    @patch('apps.lessons.services.ai.answer_course_question', return_value='4 ta javob.')
    def test_enrolled_student_can_ask_question(self, mock_answer):
        resp = self.api(self.student).post(
            f'/api/v1/courses/{self.course.id}/ask/', {'question': '2+2 nechiga teng?'}, format='json',
        )
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json()['answer'], '4 ta javob.')
        _, kwargs = mock_answer.call_args
        self.assertEqual(kwargs['subject_text'], 'Matematika')
        self.assertEqual(kwargs['lesson_titles'], ['Kvadrat tenglamalar'])
        self.assertEqual(kwargs['question'], '2+2 nechiga teng?')

    def test_non_enrolled_student_cannot_ask(self):
        from apps.accounts.models import User

        stranger = User(username='ai_s2', role=User.Role.STUDENT)
        stranger.set_password('x')
        stranger.save()
        resp = self.api(stranger).post(
            f'/api/v1/courses/{self.course.id}/ask/', {'question': 'salom'}, format='json',
        )
        self.assertEqual(resp.status_code, 403)

    def test_empty_question_rejected(self):
        resp = self.api(self.student).post(f'/api/v1/courses/{self.course.id}/ask/', {'question': ''}, format='json')
        self.assertEqual(resp.status_code, 400)
